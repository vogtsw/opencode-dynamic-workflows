    ...
    for (const phase of phaseResults) {
        ...
        if (phase.taskResults.length > 0) {
            ...
            for (const tr of phase.taskResults) {
                const tIcon = tr.status === "completed" ? "[ok]" : tr.status === "failed" ? "[fail]" : tr.status === "running" ? "[run]" : "[skip]";
...
export function generateDryRunReport(spec, totalTasks) {
    const lines = [];
...
export function generateListOutput(workflows) {
    if (workflows.length === 0) {
        return "No saved workflows found.";
    }
    const lines = ["# Saved Workflows", ""];

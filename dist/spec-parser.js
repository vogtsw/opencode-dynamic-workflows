...
export function normalizeSpec(raw) {
    ...
    if (Array.isArray(raw.phases)) {
        phases = raw.phases.map((p, i) => normalizePhase(p, i, issues));
    }
    else {
    ...
    for (const p of phases) {
        if (seenIds.has(p.id))
            issues.push(`duplicate phase id: ${p.id}`);
        seenIds.add(p.id);
...
function normalizePhase(raw, index, issues) {
    if (!raw || typeof raw !== "object") {
    ...
    if (Array.isArray(obj.tasks)) {
        tasks = obj.tasks.map((t, ti) => normalizeTask(t, ti, id, issues));
    }
    else {
...
function normalizeTask(raw, index, phaseId, issues) {
    if (!raw || typeof raw !== "object") {
...
export function validateOptions(concurrency, maxAgents) {
...
export function countTasks(spec) {
    let count = 0;
